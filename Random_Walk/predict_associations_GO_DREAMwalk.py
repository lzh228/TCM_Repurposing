...
def predict_asso(embedding_file, pair_file,clinic_predict_file_path,
                 model_checkpoint, seed, valid_ratio, test_ratio, train=True):
    set_seed(seed)
    ...
    if train:
        ...
        # )
        #
        # print(f"训练集样本量: {len(y['train'])} (正样本: {positive_count})")
        # grid_search.fit(x['train'], y['train'])
        #
        # print(f"\n最佳参数组合: {grid_search.best_params_}")
        # print(f"交叉验证最佳roc_auc: {grid_search.best_score_:.4f}")
