...
def train_edgetype_transition_matrix(em_max_iter,G,networkf,net_delimiter,walk_length,p,q):
    ...
    for i in range(em_max_iter): # EM iteration
        walks=_sample_edge_paths(G,matrices[i],walk_length,p,q)   # E step 随机采样1%的边，对每条边生成边类型转移路径
        matrices[i+1] = _update_trans_matrix(walks, matrices[i])  # M step 根据路径更新转移矩阵
...
def generate_DREAMwalk_paths(G, G_sim, trans_matrix, p,q,num_walks, walk_length,tp_factor, workers,seed):
    tot_walks = []
    nodes = list(G.nodes())

# #     print(f'# of walkers : {min(num_walks,workers)} for {num_walks} times')
    walks = parmap.map(_parmap_walks, range(num_walks),
                        nodes, G, G_sim, trans_matrix,p,q, walk_length,tp_factor,seed,
...
# parallel walks
def _parmap_walks(_, nodes,G,G_sim,trans_matrix,p,q,walk_length,tp_factor,seed):
    random.seed(seed + _)  # 使用主 seed 加上进程索引，确保不同进程种子不同
...
def save_embedding_files(netf:str, sim_netf:str, outputf:str, nodetypef:str=None, 
                         tp_factor:float=0.5, seed:int=43,
    ...
    walks=generate_DREAMwalk_paths(G,G_sim,trans_matrix,p,q,num_walks,
                                   walk_length,tp_factor, workers,seed)
    with open('results/GO-DREAMwalk_results/tmp_walk_file.pkl','wb') as fw:
