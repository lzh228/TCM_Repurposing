...
def HeterogeneousSG(use_hetSG:bool, walks:list, nodes:set, nodetypef:str, embedding_size:int, 
    ...
    # 判断路径文件HeterogeneousSG.cpp'是否存在
    if not os.path.exists('DREAMwalk/HeterogeneousSG.cpp'):
        raise FileNotFoundError('HeterogeneousSG.cpp not found in DREAMwalk directory')
    # print('HeterogeneousSG.cpp found in DREAMwalk directory')
    ...
    os.system('g++ DREAMwalk/HeterogeneousSG.cpp -o HetSG.exe -lm -pthread -O1 -march=native -Wall -funroll-loops -Wno-unused-result')
    os.system(f'HetSG.exe -train {tmp_walkf} -output {outputf} -pp {use_hetSG} -min-count 0 -size {embedding_size} -iter 5 -window {window_length} -threads {workers}')
    # compile_cmd = 'g++ DREAMwalk/HeterogeneousSG.cpp -o HetSG.exe -lm -pthread -O3 -march=native -Wall -funroll-loops -Wno-unused-result'
    # exit_code = os.system(compile_cmd)
    # if exit_code != 0:
    #     raise RuntimeError("编译失败！请检查G++环境配置和.cpp文件路径")
